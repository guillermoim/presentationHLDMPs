...
\begin{itemize}
    ...
    \item LMDPs are a <>~\citep{TodorovNIPS2007}.
...
\begin{frame}{Background - LMDPs (iii) - Eigenvector setting}
    ...
    \begin{itemize}
         \item Taking $z(s)=e^{v(s)/\lambda}$ for each $s\in\cS^+$ leads to \[ z(s) = e^{\cR(s)/\lambda} \sum_{s'}\cP(s'|s)z(s'). \]
...
\begin{frame}{Background - LMDPs (iv) - Compositionality}
    ...
    \begin{equation*}
       z(s) =  \sum_{k=1}^n w_k z_k(s) \ \forall s \in \cS.
    \end{equation*}
